...
class ModelTasks extends Model 
{
    ...
    public function editTask($id_task) {
        $stmt = $this->pdo->prepare("UPDATE `tasks` SET `name` = ?, `email` = ?, `status` = ?, `description` = ? WHERE `tasks`.`id_task` = ?");
        
        $stmt->execute([$_POST['name'], $_POST['email'], $_POST['status'], $_POST['description'], $id_task]);
    }
}
